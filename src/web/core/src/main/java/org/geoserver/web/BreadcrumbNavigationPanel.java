...
public class BreadcrumbNavigationPanel extends Panel {
    ...
    private static final CssResourceReference CSS = new CssResourceReference(BreadcrumbNavigationPanel.class, "BreadcrumbNavigationPanel.css");
    private static final JavaScriptResourceReference JS = new JavaScriptResourceReference(BreadcrumbNavigationPanel.class, "BreadcrumbNavigationPanel.js");

    public BreadcrumbNavigationPanel(String id) {
        super(id);

        LoadableDetachableModel<List<BreadcrumbItem>> breadcrumbModel = new LoadableDetachableModel<List<BreadcrumbItem>>() {
            @Override
            protected List<BreadcrumbItem> load() {
                ...
                if (resourceName == null) resourceName = currentParams.get("name").toString(null);
                if (resourceName == null) resourceName = currentParams.get("group").toString(null);
        ...
        ListView<BreadcrumbItem> breadcrumbList = new ListView<BreadcrumbItem>("breadcrumbs", breadcrumbModel) {
            ...
            protected void populateItem(ListItem<BreadcrumbItem> item) {
                ...
                LoadableDetachableModel<DropdownMenuData> menuDataModel = new LoadableDetachableModel<DropdownMenuData>() {
                    @Override
                    protected DropdownMenuData load() {
                        GeoServerApplication app = (GeoServerApplication) getApplication();
                        List<BreadcrumbContextMenuItemInfo> allBeans = app.getBeansOfType(BreadcrumbContextMenuItemInfo.class);
                        ...
                        for (BreadcrumbContextMenuItemInfo bean : allBeans) {
                            if (bean.getTargetLevel().equalsIgnoreCase(bc.getLevel())) {
                                if (bean.getAuthorizer() == null || bean.getAuthorizer().isAccessAllowed(bean.getComponentClass(), user)) {
                                    String target = bean.getTargetLevel() != null ? bean.getTargetLevel() : "LAYER";
                ...
                switch (bc.getLevel()) {
                    ...
                    case "LAYER_GROUP":
                        ...
                        break;
                    case "LAYER":
                        iconRef = new PackageResourceReference(GeoServerBasePage.class, "img/icons/silk/picture_empty.png");
                        break;
                    case "GLOBAL":
                    default:
                ...
                Class<? extends Page> targetPageClass = (bc.getPageClass() != null) ? bc.getPageClass() : GeoServerHomePage.class;
                BookmarkablePageLink<Void> link = new BookmarkablePageLink<Void>("link", targetPageClass, bc.getParameters());
                ...
                LoadableDetachableModel<List<BreadcrumbContextMenuItemInfo>> standaloneModel = new LoadableDetachableModel<List<BreadcrumbContextMenuItemInfo>>() {
                    @Override protected List<BreadcrumbContextMenuItemInfo> load() { return menuDataModel.getObject().standalone; }
                };
                ListView<BreadcrumbContextMenuItemInfo> standaloneList = new ListView<BreadcrumbContextMenuItemInfo>("standaloneItems", standaloneModel) {
                    @Override protected void populateItem(ListItem<BreadcrumbContextMenuItemInfo> menuItemListItem) {
                        populateMenuLink(menuItemListItem, getPage().getPageParameters());
                    }
                };
                contextMenuContainer.add(standaloneList);

                LoadableDetachableModel<List<CategoryGroup>> groupsModel = new LoadableDetachableModel<List<CategoryGroup>>() {
                    @Override protected List<CategoryGroup> load() { return menuDataModel.getObject().groups; }
                };
                ListView<CategoryGroup> categoryGroupsList = new ListView<CategoryGroup>("categoryGroups", groupsModel) {
                    @Override
                    protected void populateItem(ListItem<CategoryGroup> groupItem) {
                        CategoryGroup group = groupItem.getModelObject();
                        
                        groupItem.add(new Label("categoryHeader", new ResourceModel(group.category.getNameKey(), group.category.getNameKey())));
                        
                        ListView<BreadcrumbContextMenuItemInfo> categoryItemsList = new ListView<BreadcrumbContextMenuItemInfo>("categoryItems", group.items) {
                            @Override protected void populateItem(ListItem<BreadcrumbContextMenuItemInfo> menuItemListItem) {
                                populateMenuLink(menuItemListItem, getPage().getPageParameters());
                            }
                        };
    ...
    private void populateMenuLink(ListItem<BreadcrumbContextMenuItemInfo> menuItemListItem, PageParameters currentParams) {
        ...
        BookmarkablePageLink<Void> menuLink = new BookmarkablePageLink<Void>("menuLink", ctxMenu.getComponentClass(), targetParams);
        ...
        if (ctxMenu.getIcon() != null && !ctxMenu.getIcon().isEmpty()) {
            if (ctxMenu.getIcon().startsWith("/")) {
                ...
                iconComponent = new org.apache.wicket.markup.html.image.Image("menuItemIcon", 
                    new org.apache.wicket.request.resource.PackageResourceReference(ctxMenu.getComponentClass(), ctxMenu.getIcon()));
            }
        } else {
    ...
    private static class DropdownMenuData implements Serializable {
        ...
        boolean isEmpty() { return standalone.isEmpty() && groups.isEmpty(); }
    }
